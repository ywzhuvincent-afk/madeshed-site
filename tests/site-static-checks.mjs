...
includesAll(index, ['偏财机会财', "wealth:{label:'偏财运机会财专测'", 'Windfall Wealth Reading', '投资、投机、机会财', '偏财窗'], '前端偏财运产品+问大师类目+定位拓宽+每日偏财窗');
...
/* 人物档案存储层（帮家人朋友算命）：
   - 本地报告缓存必须按人物分键，否则给家人生成会覆盖自己的同类型报告；
   - buildPersonProfile 不得走 onboarding 的 calcBazi()——那个函数内部读的是
     onboarding 表单里的历法/城市状态，给别人算会错误套用你自己的设置；
   - openFortuneReport 曾有重复定义（旧版被覆盖但会吃掉补丁），必须保持唯一。 */
includesAll(index, [
  "const PEOPLE_KEY='madeshed_people_v1'",
  "const CLOUD_PEOPLE_TABLE='people'",
  'function localReportKey(type){return currentPersonKey()',
  'personId:currentPersonKey()',
  'window.MadeshedBazi.calcBaziCore(',
], '人物档案存储 + 报告按人物分键 + 请求带 personId');
assert.equal(/function buildPersonProfile\([^)]*\)\{\s*if\(typeof calcBazi/.test(index), false,
  'buildPersonProfile 不得调用 onboarding 的 calcBazi()（会套用你自己表单的历法/城市）');
